...
# are mixtures of multinomials within a period, while their marginal
# distributions remain mixtures of Binomials and therefore admit exact
# expected-cost tests.
#
# The demand model is shared with the single-item experiment. Within a
# repetition, all thirty item-count-and-drift cells are common random numbers
# of each other: the item costs, mixture weights, multinomial sequence, demand
# uniforms, starting Gamma variates, and unit innovations are all keyed by item
# index and reused, so a k-item instance is the k-item prefix of the ten-item
# one. Moving along the item-count axis then perturbs an instance instead of
# redrawing it, which is what makes the curves smooth.
...
# A candidate first mixture weight w produces the weights [w, 1-w].
const mixture_weight_values = [0.9, 0.95, 0.99]
const number_of_multinomials = 2

# Starting purchase probabilities are sampled independently for every
# multinomial.
...
function serialize_multinomial_probabilities(multinomial_probabilities)
...
function train_and_test(
    ...
    for repetition_index in 1:number_of_repetitions
        ...
        average_training_costs = rolling_origin_training_costs(
            objective_value_and_order,
            ambiguity_radii,
            weight_vector_table,
            demand_sequence,
        ...
        Threads.@threads for ambiguity_radius_index in ProgressBar(eachindex(ambiguity_radii)) # Since .pbs calls for 1 thread, this is allowed on HPC.
            for weight_parameter_index in eachindex(weight_parameters)
                weights =
                    weight_vector_table[end][weight_parameter_index]
        ...
        for weight_parameter_index in eachindex(weight_parameters)
            for ambiguity_radius_index in eachindex(ambiguity_radii)
                output_fields = (
                    ...
                    serialize_multinomial_probabilities(
...
SAA_weight_vector_table =
    precompute_weight_vector_table(windowing_weights, [history_length])
windowing_weight_vector_table =
    precompute_weight_vector_table(windowing_weights, window_size_grid)
smoothing_weight_vector_table =
    precompute_weight_vector_table(
        smoothing_weights,
...
weighted_W2_weight_vector_table =
    precompute_weight_vector_table(W2_weights, radius_ratio_grid)
...
try
    ...
    for item_count in item_counts
        ...
        repetition_mixture_weights =
            sample_repetition_mixture_weights()
